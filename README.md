...
```bash
orchestrate connections create watsonx\_api\_key
orchestrate connections configure watsonx\_api\_key --env draft --type team --kind api\_key
orchestrate connections set-credentials watsonx\_api\_key --env draft \\
...
```bash
cd wxo
orchestrate tools import -k python -f interview\_tools.py -r requirements.txt
